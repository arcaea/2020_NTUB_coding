...
public class Main {
    ...
    public static void Player_play() throws InterruptedException {
        ...
        for (int i = 0; i < Main.P;) {//i=person，對應玩家
            ...
            System.out.print("\n玩家" + user[i].getName() + "，");//輸出提示字串
            move(i);//骰骰子後移動
            if (i < Main.P - 1) i++;//換下一位
            else i = 0;//回到第一位
            System.out.print("換下一位玩家：" + user[i].getName() + "\n");//輸出提示字串

            if (user[i].getJail() == 1) {//進入防疫隔離
                Isolation(i);//呼叫防疫隔離函式
                if (i < Main.P - 1) i++;//換下一位
                else i = 0;//回到第一位
                System.out.print("換下一位玩家：" + user[i].getName()+ "\n");//輸出提示字串
            }
    ...
    ///////////////////////////////////////
    //防疫隔離---FIN
    public static void Isolation(int playerNum) throws InterruptedException {
        System.out.println("因染上疫情，休息一輪!");//輸出提示字串
        Thread.sleep(2000);//暫停2秒
        user[playerNum].setJail(0);//離開防疫隔離
    }
    ///////////////////////////////////////
    //骰子----FIN
    public static int dice() {
        ...
        System.out.println("骰到："+(d1+d2));//輸出提示字串
        return (d1+d2);//回傳總骰子數
    }
    ///////////////////////////////////////
    //破產----FIN
    public static boolean noMoney(int playerNum) throws InterruptedException {
        int cnt = 0;//變數，尚未破產人數
        boolean check=false;//確認是否破產
    ...
    public static void airMove(int playerNum) throws InterruptedException {//國內航線
        Scanner sc = new Scanner(System.in);//建立Scanner物件
        System.out.print("輸入玩家所要往前的格子數：");//輸出提示字串
        ...
        if (user[playerNum].getStay() < 35) {//當格子數<35，將經過起點
            ...
            System.out.println("玩家"+user[playerNum].getName()+"經過起點，獲得10000円");
            Thread.sleep(2000);//暫停2秒
        }
        mapMoveAfter(playerNum);//呼叫地圖上移動函式
        checkPlace(playerNum, user[playerNum].getStay());//檢查地點歸屬
    ...
    ///////////////////////////////////////
    //起點經過確認---FIN
    public static void checkPassStart(int playerNum) throws InterruptedException {
    ...
    public static void checkPlace(int playerNum, int placeNum) throws InterruptedException {
        if (pl[placeNum].getOwner()==12288) {//地點無歸屬  12288 全形空白
    ...
    ///////////////////////////////////////
    //買地----FIN
    public static void buyPlace(int playerNum, int placeNum) throws InterruptedException {
        Scanner sc = new Scanner(System.in);//建立Scanner物件
        System.out.print("玩家"+user[playerNum].getName()+"是否買" + pl[placeNum].getPlace() + "(Y-要買;N-不買)：");//輸出提示字串
        char c = sc.next().charAt(0);//取得c(Y/N)
        if (Character.toUpperCase(c) == 'Y') {//將取得字串轉大寫並判斷
            System.out.println("玩家"+user[playerNum].getName()+"沒錢，無法購買");//輸出提示字串
            Thread.sleep(2000);//暫停2秒
    ...
    ///////////////////////////////////////
    //付錢----FIN
    public static void pay(int playerNum, int placeNum) throws InterruptedException {
        ...
        switch (pl[placeNum].getOwner()) {//檢查地點歸屬
            case 65313://地點歸屬於A
                ...
                System.out.println("玩家"+user[0].getName()+"獲得過路費："+pl[placeNum].getPass_money());//輸出提示字串
                Thread.sleep(2000);//暫停2秒
                break;
            case 65314://地點歸屬於B
                user[1].setMoney(user[1].getMoney()+pl[placeNum].getPass_money());//玩家B增加錢
                System.out.println("玩家"+user[1].getName()+"獲得過路費："+pl[placeNum].getPass_money());//輸出提示字串
                Thread.sleep(2000);//暫停2秒
                break;
            case 65315://地點歸屬於C
                user[2].setMoney(user[2].getMoney()+pl[placeNum].getPass_money());//玩家C增加錢
                System.out.println("玩家"+user[2].getName()+"獲得過路費："+pl[placeNum].getPass_money());//輸出提示字串
                Thread.sleep(2000);//暫停2秒
                break;
            case 65316://地點歸屬於D
                user[3].setMoney(user[3].getMoney()+pl[placeNum].getPass_money());//玩家D增加錢
                System.out.println("玩家"+user[3].getName()+"獲得過路費："+pl[placeNum].getPass_money());//輸出提示字串
                Thread.sleep(2000);//暫停2秒
                break;
            default: break;//避免不必要的錯誤
        }
        noMoney(playerNum);//檢查破產
    }
    ///////////////////////////////////////
    //走到國有地----FIN
    public static void SOL(int playerNum, int placeNum) throws InterruptedException {
        switch (pl[placeNum].getPlace()) {//檢查國有地為何
            case "國內航線"://檢查國有地為國內航線
                ...
                if (Character.toUpperCase(q) == 'Y') {//當q為y或Y
                    if (user[playerNum].getMoney() < 50000) {//當玩家資金<50000
                        ...
                        System.out.println("玩家"+user[playerNum].getName()+"搭乘國內航線，支付50000円");
                        Thread.sleep(2000);//暫停2秒
            ...
            case "國稅廳"://檢查國有地為國稅廳
                ...
                System.out.println("玩家"+user[playerNum].getName()+"，剩餘：" + user[playerNum].getMoney());//輸出提示字串
                Thread.sleep(2000);//暫停2秒
                break;
            case "機會籤"://檢查國有地為機會籤
...
class PlaceAr {
    public static String[] place =//地名
            {"起點", "大分", "佐賀", "稻荷神社", "沖繩", "熊本", "長崎", "福岡", "九州", "宮崎", "十勝支廳", "機會籤", "檜山支廳", "渡島支廳",
                    ...
                    "國內航線", "本州", "淺草寺", "國稅廳", "東京"};
    public static char[] owner =//擁有者  12288 為全形空白
