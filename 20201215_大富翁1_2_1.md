...
public class Main {
    ...
    static Chance[] Ch = new Chance[31];
        ...
        switch (pl[placeNum].getPlace()) {//檢查國有地為何
            ...
            case "防疫隔離"://檢查國有地為防疫隔離
                if (user[playerNum].getJail() == 0)//當進入防疫隔離
                    user[playerNum].setJail(1);//防疫一天
                if (user[playerNum].getJail()== (-1)) {//玩家身上有平安御守
                    System.out.println("平安御守");
                    user[playerNum].setJail(0);
                }
                break;
            case "國稅廳"://檢查國有地為國稅廳
    ...
    public static void map(int ch) {
        ...
        String[] playerID=new String[6];
        for (int i = 0; i < 40; i++) { //儲存所有地玩家位置
            for(int z=0;z<6;z++)playerID[z] = "　";//設定playerID初始值
            int j;
            for(j = 0; j < 6; j++) {//儲存所有地玩家位置
                if(pl[j].getPass_peopleA()==1) str[i]+="Ａ"; 
                if(pl[j].getPass_peopleB()==1) str[i]+="Ｂ";
                if(pl[j].getPass_peopleC()==1) str[i]+="Ｃ";
                if(pl[j].getPass_peopleD()==1) str[i]+="Ｄ";
            }
            str[i]+="　　";
